...
class View:
    ...
    # D3D12_SAMPLER_DESC content reconstructed from the local struct assignments
    # around a CreateSampler call. None means the descriptor content did not
    # appear in the export C++; it is never default-filled, because a fabricated
    # default would pass for a measured value (the same rule as `partial`).
    sampler_desc: Optional[dict[str, Any]] = None
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.sampler_desc is not None:
            payload["sampler_desc"] = self.sampler_desc
...
class PipelineState:
    ...
    # Fixed-function state beyond the legacy flat fields. Each dict is a superset
    # of the flat fields and carries a ``parsed`` tag:
    #   full    every position was read from the CD3DX12_*_DESC constructor call
    #   partial the call carried fewer arguments than the field table
    #   default the export wrote D3D12_DEFAULT; the values are the D3D12 defaults,
    #           recorded as defaults rather than as measured values
    # The flat fields (fill_mode, cull_mode, depth_*, blend_enabled,
    # blend_states) keep their existing keys and semantics untouched.
    rasterizer: dict[str, Any] = field(default_factory=dict)
    depth_stencil: dict[str, Any] = field(default_factory=dict)
...
class DrawCall:
    ...
    # Command-list dynamic state set by OMSetBlendFactor / OMSetStencilRef. These
    # ride on the command list, not the PSO, so they are snapshotted per draw.
    blend_factor: Optional[list[float]] = None
    stencil_ref: Optional[int] = None
