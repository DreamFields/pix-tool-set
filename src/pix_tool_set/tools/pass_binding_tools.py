...
from ..engine.model import RootParameterKind, ShaderStage, ViewKind
...
def _collect(capture, draw, stage_filter: str | None, max_views: int) -> dict[str, Any]:
    ...
    for binding in draw.bindings:
        if binding.kind is RootParameterKind.DESCRIPTOR_TABLE:
            ...
            resolved = []
            # Samplers reference no resource and live in their own table, but the
            # shader reads them side by side with SRVs. Attach the draw's sampler
            # descriptors so a caller sees filter/addressing state next to the
            # texture it applies to: one sampler is a definite pairing, several
            # are reported as candidates rather than guessed.
            sampler_rows = [view.to_dict() for view in draw.samplers]
            for view in binding.resolved_views[:max_views]:
                ...
                if view.kind is ViewKind.SRV and sampler_rows:
                    if len(sampler_rows) == 1:
                        item["paired_sampler"] = sampler_rows[0]
                    else:
                        item["paired_sampler_candidates"] = sampler_rows
