...
def draw_state(args: dict[str, Any], context: ToolContext) -> ToolResult:
    ...
    data = {
        ...
        # Command-list dynamic state and the sampler descriptors in effect at
        # this draw. Samplers reference no resource; their descriptor content
        # (filter / addressing / LOD) is what matters, and stays null when the
        # export did not record it.
        "dynamic_state": {
            "blend_factor": draw.blend_factor,
            "stencil_ref": draw.stencil_ref,
        },
        "samplers": [
            view.to_dict() for view in draw.samplers[:max_views]
        ],
        "sampler_count": len(draw.samplers),
        "resource_summary": {
