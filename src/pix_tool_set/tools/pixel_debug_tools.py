...
def pixel_trace(args: dict[str, Any], context: ToolContext) -> ToolResult:
    """Superseded by pixel-history-replay; answers with a redirection, not a run.

    This tool's probe was injected but never *called*: the installer added the
    declaration and the flush and no per-draw invocation, so the trace array was
    empty on every run by construction. It also read "the currently bound render
    target", which is not a resource the caller chose, and produced one value per
    ExecuteCommandList rather than per event -- so it could not express a
    Previous/New pair at all.

    Rather than keep a tool that returns an empty list and looks like a measurement,
    it reports what happened and names the tool that does the job. Failing loudly is
    the only option that cannot be mistaken for "the pixel was never written"; the
    old behaviour was exactly that mistake waiting to happen.
    """
    capture = context.capture(args)
    x = int(args.get("x", 0))
    y = int(args.get("y", 0))
    ...
    # The candidate resources are still worth computing: they are what the caller
    # needs in order to pick --resource-id for the replacement tool, and they cost
    # nothing but a metadata walk.
    covering: list[dict[str, Any]] = []
    for rid, info in capture.resource_usage.items():
        resource = capture.resource(rid)
        if resource is None or not resource.width or not resource.height:
            continue
        if x >= resource.width or y >= resource.height:
            continue
        writers = sorted(set(info.get("write_draws", [])))
        if not writers:
            continue
        covering.append(
            {
                ...
                "writing_event_count": len(writers),
                "is_render_target": bool(info.get("render_target_draws")),
                "is_depth": bool(info.get("depth_draws")),
            }
        )
    covering.sort(key=lambda entry: -entry["writing_event_count"])

    data: dict[str, Any] = {
        "pixel": {"x": x, "y": y},
        "superseded_by": "pixel-history-replay",
        "why": (
            "This tool's probe was never invoked per draw, so its trace was always "
            "empty, and it sampled whatever render target happened to be bound rather "
            "than a resource the caller named. Neither can produce the Previous/New "
            "value pair a pixel history needs."
        ),
        "resources_covering_pixel": covering[:20],
        "trace": [],
        "entry_count": 0,
    }
    ...
    result.add_diagnostic(
        "warning",
        "pixel-trace no longer runs a replay. Use pixel-history-replay --resource-id "
        f"<id> --x {x} --y {y}, which records a copy immediately before and after each "
        "event and reports both values with a per-event verdict.",
    )
    if covering:
        best = covering[0]
        result.add_diagnostic(
            "info",
            f"{len(covering)} resource(s) cover ({x}, {y}) and are written this frame. "
            f"The most written is resource {best['resource_id']} "
            f"({best['format']}, {best['writing_event_count']} writing events): "
            f"pixel-history-replay --resource-id {best['resource_id']} --x {x} --y {y}",
        )
    else:
        result.add_diagnostic(
            "info",
            f"No resource that covers ({x}, {y}) is written in this frame, so there "
            "would be no events to sample.",
        )
    return result
