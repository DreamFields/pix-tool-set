...
from ..engine import activity, exportstate, framesnapshot, override as override_mod, screencap, uavprobe
...
def replay_reset(args: dict[str, Any], context: ToolContext) -> ToolResult:
    ...
    # 1b. State-level overrides (replay-override) restore from their own per-file
    #     backups. This runs before the .orig restore below: the .orig is the
    #     pristine session-open file, and override backups were taken over
    #     whatever state existed when the override was applied.
    overrides_restored = override_mod.restore_overrides(root)
