...
启动
 ...
 └─ pix-tool-set env-check             （新机器/首次运行）确认本机依赖齐备
      ├─ data.ready.read_only_analysis=false → 按 data.next_step 提示用户补依赖，不要继续
      ├─ data.ready.gpu_replay=false        → 静态分析照常，但避免调 replay/read-uav 类工具
      └─ data.checks[].fix                  → 每条失败自带修复动作，可直接转述给用户
...
| `pixtool_missing` | 找不到 pixtool.exe | 调 `env-check` 拿到完整缺失清单与修复动作，或提示安装 PIX / 传 `--pixtool` |
...
| `disassembly_unavailable` | dxcompiler.dll 不可用 | 调 `env-check --scope core` 确认原因（未装 PIX / DLL 加载失败），并告知用户此类字段本机不可得 |
| `compiler_unavailable` | 没有可用的 HLSL 编译器 | 调 `env-check --scope core`；提示安装 PIX（含 dxcompiler.dll）或 Windows SDK（含 dxc.exe） |
| `cmake_configure_failed` / `cmake_build_failed` | 重建导出工程失败 | 调 `env-check --scope replay`：通常是生成器不匹配（按 `recommended_generator` 传 `--generator`）或 Agility SDK 缺失 |
